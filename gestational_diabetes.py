...
gdm_full =gdm_full.drop(["HDL","OGTT","Hemoglobin"], axis=1)
print(gdm_full.columns)

#plotting the distribution of the columns with missing values
'''fig, ax = plt.subplots(1,2)

ax[0].hist(gdm_full['BMI'])
#ax[0,1].hist(gdm_full['HDL'])
ax[1].hist(gdm_full['Sys BP'])
#ax[1,1].hist(gdm_full['OGTT'])

ax[0].set_title('BMI')
ax[1].set_title('Sys BP')

# Set the main title for the whole figure
fig.suptitle('Distribution of BMI, Sys BP', fontsize=12)
...
#dropped ( can't easily be done at home)
...
# checking outliers
cols_1 = gdm_full[['Age', 'No of Pregnancy',
...
cols_3 = gdm_train[['Sys BP', 'Dia BP', 'Hemoglobin', 'Sedentary Lifestyle',
       'Prediabetes']]
cols_1.plot(kind = "box" , subplots = True, layout =(1,5), figsize=(5,10), sharey = False, sharex= False)
plt.suptitle("Boxplots of Age, No of Pregnancy, Gestation in Previous Pregnancy and BMI")
...
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.show()
cols_3.plot(kind = "box" , subplots = True, layout =(1,5), figsize=(5,10), sharey = False, sharex= False)
plt.suptitle('Boxplots for Sys BP, Dia BP, Hemoglobin, Sedentary Lifestyle and Prediabetes')
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.show()

columns_with_outliers = ['BMI', 'Sys BP', 'Dia BP', 'Hemoglobin']
...
'''''
'''# Optional: If true labels are available, evaluate the predictions

if 'Class Label(GDM /Non GDM)' in gdm_test.columns:
    y_gdm_test = gdm_test['Class Label(GDM /Non GDM)']
    test_set_mse = MSE(y_gdm_test, y_gdm_pred)
    print(f'Aug Test Set Mean Squared Error: {test_set_mse}')
    test_set_rmse = test_set_mse ** (1/2)
    print('Aug Test Set RMSE: {:.2f}'.format(test_set_rmse))
    y_gdm_pred_binary = [1 if pred >= 0.5 else 0 for pred in y_gdm_pred]
    aug_accuracy = accuracy_score(y_gdm_test, y_gdm_pred_binary)
    print(f'Aug Test Set Accuracy: {aug_accuracy}')
else:
    print('Predictions on  Test Set:', y_gdm_pred)'''